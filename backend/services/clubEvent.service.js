...
const clubEventService = {
    createEvent(clubId, userId, { title, description, target_distance, target_duration_seconds, start_time, end_time }) {
    ...
    joinEvent(clubId, userId, eventId) {
        ...
        if (!clubWithMemberStatus || clubWithMemberStatus.status !== 'approved') {
            const error = new Error('You must be an approved member of the club to join its events');
            error.status = 403;
            throw error;
        }
